...
export default function TeamItem({ data, title }: propsType) {
  return (
    <div>
      <div className="my-5">
        <div className=" font-bold py-4 md:text-[40px] pl-10">{title}</div>
        <div className="grid grid-cols-3 lg:grid-cols-4  xl:grid-cols-5 px-3 justify-items-center gap-5">
          {data.map((item, index) => {
            return (
              ...
              >
                <img
                  className=" w-full h-[6rem] sm:h-[10rem] xl:h-[13rem]"
                  src={getAssetUrl(`team/${item.img}.jpg`)}
                  alt=""
                />
                <div className=" py-2 text-center sm:text-[1.5rem] xl:text-[2rem]">
                  {item.name}
                </div>
              </div>
            );
